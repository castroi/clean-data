...
def test_handles_text_message(bot: SignalBot):
    ...
    bot.handle_message(sender="42cc8f08-8b0a-4a79-a46d-000000000001", message="hello", attachments=None)
...
def test_handles_unsupported_file(bot: SignalBot):
    ...
    bot.handle_message(sender="42cc8f08-8b0a-4a79-a46d-000000000001", message="", attachments=[attachment])
...
def test_send_attachment_sends_bytes(bot: SignalBot, tmp_path: Path):
    ...
    bot._send_attachment("42cc8f08-8b0a-4a79-a46d-000000000001", test_file, "Done.")

    bot._api.send_message.assert_called_once_with(
        message="Done.",
        recipients=["42cc8f08-8b0a-4a79-a46d-000000000001"],
...
def test_send_attachment_missing_file(bot: SignalBot, tmp_path: Path):
    ...
    bot._send_attachment("42cc8f08-8b0a-4a79-a46d-000000000001", missing_file, "Done.")
...
def test_allowlist_rejects_unauthorized_sender_silently(tmp_path: Path):
    ...
    cfg = _make_config(tmp_path, allowed_senders="42cc8f08-8b0a-4a79-a46d-000000000001")
    with patch("signal_bot.SignalCliRestApi"):
        b = SignalBot(cfg)
        b._api = MagicMock()

    b.handle_message(sender="aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", message="hello", attachments=None)
...
def test_allowlist_allows_authorized_sender(tmp_path: Path):
    ...
    cfg = _make_config(tmp_path, allowed_senders="42cc8f08-8b0a-4a79-a46d-000000000001, 11111111-2222-3333-4444-555555555555")
    with patch("signal_bot.SignalCliRestApi"):
        b = SignalBot(cfg)
        b._api = MagicMock()

    b.handle_message(sender="42cc8f08-8b0a-4a79-a46d-000000000001", message="hello", attachments=None)
...
def test_allowlist_allows_second_authorized_sender(tmp_path: Path):
    """Bot with ALLOWED_SENDERS set should process messages from authorized senders."""
    cfg = _make_config(tmp_path, allowed_senders="42cc8f08-8b0a-4a79-a46d-000000000001, 11111111-2222-3333-4444-555555555555")
    with patch("signal_bot.SignalCliRestApi"):
        b = SignalBot(cfg)
    ...
    b.handle_message(sender="11111111-2222-3333-4444-555555555555", message="hello", attachments=None)
